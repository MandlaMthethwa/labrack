...
if($_SERVER["REQUEST_METHOD"] == "POST"){	
    ...
    if(empty($username_error) && empty($password_error)){
    	...
    	$sql = "SELECT lecturerID, lecturerPassword FROM lecturer WHERE lecturerUsername = ?";
    	...
    	if($stmt = $conn->prepare($sql)){
    		...
    		if($stmt->execute()){
    			...
    			if($stmt->num_rows == 1){
    				...
    				$stmt->bind_result($lecturer_id, $hashed_password);
    				if($stmt->fetch()){
...
<body>
 <div class="lecturer-form-content">
